...
- IPv6 [X]
- Unix Support [X]
- Windows Support [X] (untested)
- Multicast Support [X] (currently IPv4 only)
- Minimal [X] (2 dependencies, compiled size ~300kb)
...
```
cargo run --bin server --release -- --port 9920 --client_addr 127.0.0.1 --client_addr 127.0.0.2
```
...
```
cargo run --bin client --release -- --port 9920 --listen_addr 0.0.0.0/0 --path /dev/random
```
